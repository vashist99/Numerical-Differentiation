...
vector<lld> forward(lld val,vector<lld> h ){
    ...
    for(int i=0;i<h.size();i++){
        ...
        et.push_back(exp2(t[j]));
    }

     lld del1y0 = et[1] - et[0];
     lld del2y0 = et[2] - 2*et[1] + et[0];
     lld del3y0 = et[3] - 3*et[2] + 3*et[1] - et[0]; 

        p = (val - t[0])/(h[i]);
        lld calculated_derivative = (del1y0+((2*p-1)*del2y0/2)+((3*p*p-6*p+2)*del3y0/6))/(h[i]);
        lld error = abs(calculated_derivative -  exp2(val));
        err.push_back(error);
    }
...
lld N(lld h,lld val,lld t0,lld t1,lld t2,vector<lld> et){
    lld del1y0 = et[1] - et[0];
    lld del2y0 = et[2] - 2*et[1] + et[0];
    lld del3y0 = et[3] - 3*et[2] + 3*et[1] - et[0];
    
    return et[0] + ((val-t0)*del1y0/h) + ((val-t0)*(val-t1)*del2y0/(2*h*h)) + ((val-t0)*(val-t1)*(val-t2)*del3y0/(6*h*h*h));
}

vector<lld> extra(lld val,vector<lld> h){

    //calclating the cosine values for the corresponding ts:
    //val is the point at which the derivative has to be found out
    vector<lld> et;
    vector<lld> t;
    ...
    for(int i=0;i<h.size();i++){
        ...
        lld term1 = N(h[i],(val+(h[i]/4)),t[0],t[1],t[3],et);
        lld term2 = N(h[i],(val-(h[i]/4)),t[0],t[1],t[3],et);
        lld term3 = N(h[i],(val+(h[i]/2)),t[0],t[1],t[3],et);
        lld term4 = N(h[i],(val-(h[i]/2)),t[0],t[1],t[3],et);
